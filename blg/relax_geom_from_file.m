function [atom_pos_shift] = relax_geom_from_file(rot_theta,sc_b1,sc_b2,top_layer,atom_pos_list)
...
for i = 0:max_k
    for j = 0:max((i-1),0)
        ...
        if (i == 0)
           ...
           r_max = 0; 
           pre_fac = 1.0; % [0,0] comp has no cos/sin expansion.
        end
        ...
        for r_idx = 0:r_max
            ...
            disp_x = disp_x + c(1)         * pre_fac*sin(k1(1)*atom_pos_list(:,1) + k1(2)*atom_pos_list(:,2));
            disp_y = disp_y + c(2)         * pre_fac*sin(k1(1)*atom_pos_list(:,1) + k1(2)*atom_pos_list(:,2));
            disp_z = disp_z + coeffs(idx,3)* pre_fac*cos(k1(1)*atom_pos_list(:,1) + k1(2)*atom_pos_list(:,2));
