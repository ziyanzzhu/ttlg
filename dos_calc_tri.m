...
%         E_arr: the list of energy to sample
...
function fname=dos_calc_tri(q1_list,q2_list,E_arr,k_cutoff,w_inv,nq,num_eigs,E_list,q_cut_type,monoG)

    monoG = 1;   % normalize by the monolayer DOS
    linecut = 0; % if on, calculate the DOS for a line (useful for debugging)
    run_idx = 1; 
    
    for e_idx = 1:length(E_arr)
        E_field = E_arr(e_idx);
        for q1_idx = 1:length(q1_list) 
            for q2_idx = 1:length(q2_list)
                theta_list = [q1_list(q1_idx) 0 q2_list(q2_idx)];
                fname = ['dos_q12_' num2str(abs(theta_list(1))) '_q23_' num2str(abs(theta_list(3)))...
                            '_kcut_' num2str(k_cutoff) '_qtype_' num2str(q_cut_type) '_nq_' ...
                            num2str((nq+1)^2) '_D' num2str(E_field) '_zip.mat'];
                if exist(['./data/' fname ])
                    load(['./data/' fname ])
                else 
                     % twisting angles (global)
                    ...
                    if monoG
                        ...
                        % fit the slope 
                        cond = E_list>=-E_field*2& E_list < 100e-3-E_field*2;
